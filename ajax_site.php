...
if($a == "save_edit") { // сохранить изменения, но не закрывать
	saveit();
	$a="view";
}
...
// function l(id) { zabil(id,'...wait...'); ajax_site('view',id,0,0); }
...
function ajax_site(a,id,name,text) { JsHttpRequest.query('".$wwwhost."ajax_site.php',
{ action: a, id: id, name: name, text: text, hash: '{hashpage}' },
function(responseJS, responseText) { if(responseJS.status) { return responseJS.otvet; } },true);
}
...
if ($admin && $a == "ans") otprav_add('textarea','Answer'); // рвызвать на редактирование ответ на комментарий
