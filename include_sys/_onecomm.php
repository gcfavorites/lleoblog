...
function comment_one($p,$mojno_comm,$level=false) { $lev=$level*$GLOBALS['comment_otstup'];
	...
	$c=comment_prep($p,$mojno_comm,$level); // подготовить данные
...
if(($GLOBALS['admin'] and !empty($_GET['nocache'])) or $mas===false) { // ------------ если нет в кэше, то прочесть ------------
	...
	cache_set($cachename,$mas,$GLOBALS['ttl_longsite']); $s=''; // запомнить в кэш
...
or (( strstr($GLOBALS['BRO'],'Yandex') || $GLOBALS['IP']=='78.110.50.100') and intval($m['value'])==120530 ) // хитрую гайку показывать €ндексу
) // открыт ли?
