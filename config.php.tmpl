...
# строка для хэширования пароля (любая, от балды)
	$koldunstvo = "мыпи сали мыпи сали наши пальчики устали";
	$hashinput = "пока мы не знаем закона природы, он существуя и действуя"; // строка для вывода хэшиков каждой страницы
	$hashlogin = "этокатастрофаШеф"; // допаролька логона сайта - если поменять, отвалятся все залогиненные юзеры


# memcache - по умолчанию отключен, удалите ВООБЩЕ эту строчку, если он у вас установлен
$memcache = false;
...
############ логины ###########################################################################
$db_login="login"; // имя базы логинов
#########################################################################################
...
$foto_logo=chr(169)." ".chr(171)."Блог Леонида Каганова http://lleo.aha.ru/blog/".chr(187); // автоматическая подпись фоток
