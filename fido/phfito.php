...
$SRC_PATH = 'phfito'; $SRC_PATH = rtrim($SRC_PATH,"\\/").'/';
...
function run_phfito_run($argc, $argv)
{
	...
	if (isset($opt['c'])) {
		if ($opt['c'][0] == '-') { $CONFIG_FILE = false; } else { $CONFIG_FILE = $opt['c'][0]; }
	}
	...
	if (isset($opt['u'])) $COLOUR = $opt['u'][0];

	if ($COLOUR == 'always' || $COLOUR == 'on') $COLOUR = true;
	else if ($COLOUR == 'never' || $COLOUR == 'off') $COLOUR = false;
	else if (function_exists('posix_isatty')) $COLOUR = posix_isatty(1);
	else $COLOUR = false;

	chdir($BASE_DIR);

	$lch = $opt_s || $opt_t || $opt_m || $opt_p || $opt_X || $opt_y || $opt_e || $opt_W || $opt_P || $opt_R || $opt_r;
	$tch = $opt_t || $opt_y || $opt_s || $opt_p || $opt_X || $opt_e || $opt_P || $opt_r || $opt_R;
	...
	if ($opt_X) { foreach($opt['X'] as $include) { eval(file_get_contents($include)); } }
	...
	if (isset($opt['h'])) {	$cmd->printhelp(); }
	if (isset($opt['v'])) {	$cmd->printver(); }
...
$str = split(' ', 'phfito -ts');
return run_phfito_run(sizeof($str), $str);
#return run_phfito_run($_SERVER['argc'], $_SERVER['argv']);
*/
/***************************************************************************/
//                                 Resources                               //
...
';
	...
	function printver()
	{
		if (preg_match("/\s+(\S+)\s+/", '$Revision: 1.13 $', $match)) {
			$revision = $match[1];
		} else {
			$revision = 'unknown';
		}
