...
//
/    $Id: L_ziplib.php,v 1.2 2007/10/14 08:29:54 kocharin Exp $
\*/
...
function gzip_compress ($data) {
  ...
  if (!($fp = gzopen($filename, "wb")))
      exit("gzopen failed - writing $filename");
...
function gzip_uncompress ($data) {
	GLOBAL $temp_dir;
  $filename = $temp_dir.'/temp.tmp';#gzip_tempnam();
  if (!($fp = fopen($filename, "wb")))
      exit("fopen failed: 2, $filename");
  fwrite($fp, $data, strlen($data));
  if (!fclose($fp))
      exit("fclose failed");

  if (!($fp = gzopen($filename, "rb")))
      exit("gzopen failed - reading $filename");
  $unz = '';
...
function zip_deflate ($content)
{
  ...
  if ($comp_type != GZIP_DEFLATE)
      exit("Bad gzip comp type");
  if (($flags & 0x3e) != 0)
      exit(sprintf("Bad flags (0x%02x)", $flags));

  $gz_header_len = 10;
  $gz_data_len = strlen($z) - $gz_header_len - 8;
  if ($gz_data_len < 0)
      exit("not enough gzip output?");
...
function zip_inflate ($data, $crc32, $uncomp_size)
{
  if (!function_exists('gzopen'))
      exit("Can't inflate data: zlib support not enabled in this PHP");
...
//    header("Content-Disposition: attachment; filename=\"$zipname\"");
  ...
  function addRegularFile ($filename, $content, $attrib = false) {
    ...
    $ati = (isset($attrib['is_ascii'])&&($attrib['is_ascii'])) ? 1 : 0;
  ...
  function finish () {
    ...
    fwrite($this->file,$this->dir);

    // Construct the "End of central directory record"
    fwrite($this->file,ZIP_ENDDIR_MAGIC);
    fwrite($this->file,pack("vvvvVVv",
          ...
          strlen($this->comment)));
    fwrite($this->file,$this->comment);
  }
}

class ZipToBrowser
{
  function ZipToBrowser ($comment = "", $zipname = "archive.zip") {
	GLOBAL $CONFIG;
    $this->comment = $comment;
    $this->nfiles = 0;
    $this->dir = "";        // "Central directory block"
    ...
    $zipname = addslashes($zipname);
	if (isset($CONFIG) && $CONFIG->GetVar('manual_headers')) {
 ...
 * BUGS:
 *
 * Many of the exit()'s should probably be warn()'s (eg. CRC mismatch).
 *
...
class ZipReader
{
  ...
  function ZipReader ($zipfile) {
    ...
    exit("Can't open zip file '$zipfile' for reading");
  }

  function _read ($nbytes) {
    $chunk = fread($this->fp, $nbytes);
    if (strlen($chunk) != $nbytes)
    exit("Unexpected EOF in zip file");
    return $chunk;
  }
...
//    return fclose($this->fp);
  ...
  function readFile () {
    ...
    if ($magic != ZIP_CENTHEAD_MAGIC)
        exit("Bad header type: " . htmlspecialchars($magic)); // FIXME: better message?
    return $this->done();
      }
    if (($flags & 0x21) != 0)
    exit("Encryption and/or zip patches not supported.");
    if (($flags & 0x08) != 0)
    exit("Postponed CRC not yet supported."); // FIXME: ???
    
    if (!$filename_len) { return ""; }
    ...
    $crc = zip_crc32($data);
    if ($crc32 != $crc)
        exit(sprintf("CRC mismatch %x != %x", $crc, $crc32));
      }
    else
    exit("Compression method $comp_method unsupported");

    if (strlen($data) != $uncomp_size)
    exit(sprintf("Uncompressed size mismatch %d != %d",
...
function ParseMimeifiedPages ($data)
{
    ...
    if ($encoding == 'quoted-printable')
        ...
        exit( sprintf(_("Unknown %s"), 'encoding type: $encoding') );
