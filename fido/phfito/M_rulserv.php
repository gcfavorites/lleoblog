...
//
/    $Id: M_rulserv.php,v 1.4 2011/01/08 12:21:09 kocharin Exp $
\*/
/*
class M_rulesserver
{
	...
	function hook_netmail($params)
	{
		...
		if ($forus && (strcasecmp($CurrentMessage->ToUser,'rulesserver')==0)) {
			...
			for ($i=0,$s=sizeof($lines);$i<$s;$i++):
				$line = $lines[$i];
				if ($line = trim($line)):
					tolog('t',"Rulesserver command: $line");
					$msgbody.= "\r".$qp.$line."\r\r".$this->serv_cmd($line)."\r";
				endif;
			endfor;
			$lines = array();
			$msgps.= $this->serv_done();
			SendReply($this->msgid,$this->to,$msgbody,$msgps,MSG_PVT,$this->user,'Reply from RulesServer',false,'Phfito FAQ Server');
		}
	}
		...
		if (!$this->cache) {
			...
			foreach($CONFIG->GetArray("faqtopic") as $topic) {
				...
				if (!isset($file_enc)) {
					...
					$file_enc = strtolower($file_enc);
					if (in_array($file_enc,array('d','dos')) || (strpos($file_enc,'866') !== false)) {
						$file_enc = 'd';
					} else if (in_array($file_enc,array('k','koi','koi8','koi-8','koi8-r','koi8r'))) {
						$file_enc = 'k';
					} else if (in_array($file_enc,array('w','windows','win')) || (strpos($file_enc,'1251') !== false)) {
						$file_enc = 'w';
					} else if (in_array($file_enc,array('m','mac'))) {
						$file_enc = 'm';
					} else if (in_array($file_enc,array('i','iso'))) {
						$file_enc = 'i';
					} else {
						$file_enc = 'd';
					}
