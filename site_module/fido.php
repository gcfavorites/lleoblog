...
<p>Исправь свою карточку и повтори: <span class=l onclick=\"majax('login.php',{action:'openid_form'})\">".$GLOBALS['imgicourl']."</span>";
	} else 
...
<br>можно зарегистрироваться как Openid, кликнув <span onclick=\"majax('login.php',{action:'oldlogin_form'})\" class='l'>сюда</span>.

<p>Если нет, открой свою учетную карточку <span class=l onclick=\"majax('login.php',{action:'openid_form'})\">".$GLOBALS['imgicourl']."</span>
<br>и заполни поля: 'Логин' (только латинские строчные буквы),
<br>'Пароль', 'email' и 'Ник' (имя и фамилия латинскими буквами,
<br>это поле нужно для регистрации в ФИДО).
...
if($a=='loadareas') { // область арий скачать
	...
	$z=($fido_point && !RE('all')); // поинт ли это?
	$a=array();
...
<div><input type=button value='Сохранить' onclick=\"majax('module.php',{mod:'fido',a:'settings_change',msg_new:idd('m_msg_new').value,msg_reply:idd('m_msg_reply').value})\"></div>";
...
- только мои | javascript:majax(\"module.php\",{mod:\"fido\",a:\"loadareas\"})
...
- движок сайта | http://lleo.aha.ru/blog/lleoblog
