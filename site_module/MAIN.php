...
SCRIPTS("logout","function logout2() {
	if(!confirm(realname+' действительно хочет разлогинитьс€?')) return;
...
// SCRIPT_ADD($GLOBALS['www_js']."pravka_blog.js"); уже не надо, € все хоз€йство (6 кб в main.js перенес)
...
$conf=array_merge(array(
	...
	'next'=>"<a href='{nextlink}'>следующа€ заметка &gt;&gt;</a>",
	'no'=>"&nbsp;",
...
<p class=br><blink>стерлась авторизаци€!</blink></p>
<b>".h($ISE['openid'])."</b><br><input type=button value='¬осстановить!' onclick=\"majax('login.php',{action:'oldlogin_form',login:'".h($ISE['openid'])."'})\">
</div>
...
<p class=br><blink>стерлась авторизаци€!</blink></p>
<b>".h($ISE['login'])."</b>
<br><input type=button value='¬осстановить!' onclick=\"majax('login.php',{action:'oldlogin_form',login:'".h($ISE['login'])."'})\">
</div>

"; 

// <p class=br>пароль:</p><input type=text size=10 id='loginobr_unic11p'>
// <br><input type=button value='¬осстановить!' onclick=\"zabil('openidotvet','<div class=o>соединение...</div>');majax('login.php',{action:'openid_logpas',rpage:mypage,mylog:'".$ISE['openid']."',mypas:idd('loginobr_unic11p').value});\">
...
function ANOTHER_DATE() { global $article; $s='';
	...
	if($pp!==false && sizeof($pp)) {
	   ...
	   return "<div style='text-align: left; border: 2px dashed #ccc; margin: 10px 10px 20px 10px; padding: 10px;'><i>ƒругие записи за это число:</i>".$s."</div>";
	}
    }
return '';
}
...
function MAY9() { global $article; //--- георгиевска€ ленточка ---
	$m9=intval(date("md")); 
...
//otprav("	return ($article["cat"]!='')?"<p style='font-size: 10pt; margin-top:4px;'>–убрика: <a style='font-size: 10pt;' href='".$httphost."blog?cat=".$article["cat"]."'>".$article["cat"]."</a></p>":'';}
*/
