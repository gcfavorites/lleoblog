...
if(!$mode) { // запомнить последние параметры
	...
	if($admin) if(!file_put_contents($admin_comment_last,$l)) $er[]="не удается записать файл <b>$admin_comment_last</b>, проверьте права папки на запись";
}
...
$sqlref="SELECT * FROM `dnevnik_comments` WHERE ";
	if($mode=='one') $sqlref .= "(`login`='".e($_GET['user'])."' OR `speckod`='".e($_GET['sc'])."') AND ";
	if($mode=='onelogin') $sqlref .= "`login`='".e($_GET['user'])."' AND ";
	if($mode=='onesc') $sqlref .= "`speckod`='".e($_GET['sc'])."' AND ";
	if($ncom!='-') $sqlref.="DateTime>".$lastcom." ORDER BY DateTime";
	else $sqlref.="DateTime<".$lastcom." ORDER BY DateTime DESC";
$sqlref.=" LIMIT ".($lim+1);
...
if($colnewcom>$lim) { $colnewcom--; unset($sql[$colnewcom]); }
	...
	$a=date('Y-m-d_H-i-s',$sql[0]['DateTime']);
	$b=date('Y-m-d_H-i-s',$sql[$colnewcom-1]['DateTime']);
	if($ncom!='-') { $l=$a; $a=$b; $b=$l; }
...
if($colcom<=$lim) if($ncom=='-') $linkprev_x=''; else { $link_x='<br>на этом всё пока'; }
	...
	for($i=0;$i<$colnewcom;$i++) if(isset($sql[$i])) {
		$p=$sql[$i];
		$data=$p['Date']; $prostynka .= print_header($data);
...
$linkprev_link=$wwwhost."comments?".makeGET(array('ncom'=>'prev','lastcom'=>urlencode($linkprev_x)));
$linknext_link=$wwwhost."comments?".makeGET(array('ncom'=>'next','lastcom'=>urlencode($linknext_x)));
...
// function screen_comments($id) { return msq_update('dnevnik_comments',array('metka'=>'screen'),"WHERE `ID`='$id'"); } // пометить в базу
// function del_comments($id) { return msq_del('dnevnik_comments',array('ID'=>"$id")); } // удалить из базы
...
function print_header($data) {
	$p=ms("SELECT `DateDate`,`Header`,`view_counter` FROM `dnevnik_zapisi` WHERE `Date`='".e($data)."' LIMIT 1",'_1');
	$x=$p['Header']; if($x=='') $x='(&nbsp;)';
	return "<p><b><a href=".$GLOBALS['wwwhost'].$data.($p['DateDate']?".html":'').">".$data." - ".$x."</a></b> (счетчик: ".$p['view_counter'].")";
}

function print1($p) { global $admin;
	$id=$p['id'];
	return "\n\n<a name='c".$id."'><div class=item_com name=".$id.($admin?" id=".$id:"").">\n".onecomment_ans($p,$p['idza'])."</div>";
}
