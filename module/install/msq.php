...
function parse_msqtxt($txt,$ta='') { $s=file_get_contents($txt); // взять список баз на создание
...
//=============================================
function parse_mytables($ta='') { $o=''; $i="<img src='".$GLOBALS['www_design'].'e3';
...
."</div>";
		...
		if(sizeof($arr_change)) { $s=''; foreach($arr_change as $n=>$k)
			$s.="<div>".msqtableknop('change_field',$tab,$n,$k[1],$k[0])."</div>"; $g.="<div>$s</div>";
		}
...
." title='Delete Table'>"
.(msq_table($tab.'_old')?" $i/document-revert.png'"
...
." data: ".msq_info_table($tab,'Update_time')."'>":'')
...
$o.="<div>
<span style='color: #779933; font-size:18px; text-decoration: bold; cursor:pointer;'"
.(sizeof($arr_ok)?" onclick=\"tudasuda('msqtok_$tab')\"":'').">$i/expand_plus.gif'>".h($tab)
." (".ms("SELECT COUNT(*) FROM `".e($tab)."`","_l").")</span>" // число элементов
." <span"
