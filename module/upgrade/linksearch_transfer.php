...
// ѕерекачать старую базу комментариев
...
if($mypage1 && $pp!==false && sizeof($pp)) { $s .= admin_rereload($action,$Nskip,5); $s.= "<p>¬сего дл€ обработки: ".sizeof($pp)."<p class=br>";
...
$pp=ms("SELECT * FROM `$basa` WHERE `DateID`='0' LIMIT $Nskip","_a",0);
if($mypage2 && $pp!==false && sizeof($pp)) { $s .= admin_rereload($action,$Nskip,5); $s.= "<p>¬сего дл€ обработки: ".sizeof($pp)."<p class=br>";
